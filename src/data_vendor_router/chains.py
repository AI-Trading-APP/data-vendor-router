...
Defaults:
  ohlcv:        polygon → tiingo → alpaca → openbb → yfinance
  news:         newsapi → tiingo → benzinga → alpha_vantage → yfinance
  fundamentals: yfinance → alpha_vantage → polygon → openbb
...
and should only ever be an inert final fallback. openbb sits at slot-4 as the
last working OHLCV source on the VPS when polygon/tiingo/alpaca breaker-open.
The env-var override (DVR_OHLCV_PRIORITY) still takes full precedence over
this default.
"""
